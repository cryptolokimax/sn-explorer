    ...
    return (
        ...
        <Box align="center" justify="center">
            <Box align="center" justify="center" pad="small">
                <Heading>{(height === 385824) ? 'Trusty Tyr hardfork' : 'This block'} {inFuture ? 'will happen' : 'happened'}</Heading>
            <Heading size="large">
                {
                 blockDiff === 0 ? 'just now' : 
                 (inFuture ? 
                    <span>in <NumberFormat value={blockDiff} displayType={'text'} thousandSeparator={true} /> {pluralize('block', blockDiff)}</span> : 
                    <span><NumberFormat value={(blockDiff * -1)} displayType={'text'} thousandSeparator={true} /> {pluralize('block', (blockDiff * -1))} ago</span>
                 )
                }
            </Heading>
            <Heading>
            ...
            <p>
                <Text>
                    Current height: <NumberFormat value={currentHeight} displayType={'text'} thousandSeparator={true} />
                </Text>
            </p>
            </Box>
        </Box>
