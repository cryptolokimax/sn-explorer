...
const LabelledChart = ({ maxFreq, label, value, onHover }) => (
  <Box flex={false}>
    <Chart
      type="bar"
      bounds={[
      ...
      values={[
        {
          value: [1, value],
          onHover: (over) =>
            over ? onHover(label, value) : onHover(null, null),
        },
      ]}
      round
      gap="xxsmall"
...
const OperatorFeeDistribution = ({ feeHistogram }) => {
  ...
  const maxFreq =
    (_.maxBy(feeHistogram, (o) => o.frequency).frequency / totalElements) * 100;
  ...
  const outliers = classRange.reduce(
    (a, c) => {
      if (c > 20) {
        ...
        if (a.inARowCount < 3)
          return { start: c + 1, frequency: 0, inARowCount: 0 };
        return {
          ...a,
          frequency: a.frequency + frequency,
          inARowCount: a.inARowCount + 1,
        };
      }
      return a;
    },
          ...
          classRange
            ...
            .map((c) => {
              ...
              return (
                <LabelledChart
                  onHover={(l, v) =>
                    setHighlightedBlock({ label: l, value: v })
                  }
                  key={c}
                  maxFreq={maxFreq}
      ...
      {highlightedBlock && highlightedBlock.label !== null && (
        ...
        >
          ...
          <Text size="small">
            of Service Nodes
            <br />
            have {highlightedBlock.label}% operator fee
          </Text>
        </BoxNoPointer>
      )}
    </Stack>
