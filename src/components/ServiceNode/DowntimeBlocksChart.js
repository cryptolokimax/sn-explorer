...
import { Chart, Stack, Box, Text } from "grommet";
...
const DowntimeBlocksChart = ({ downtimeBlocksHistories }) => {
  ...
  const chartData = downtimeBlocksHistories.map((d, i) => ({
    ...
    label: d.createdAt,
    onHover: (over) =>
      over ? setHighlightedBlock(d) : setHighlightedBlock(null),
  }));
  return (
    <Stack
      anchor="center"
    ...
    >
      <Chart
        ...
        bounds={[
          [0, numOfBlocks - 1],
          [minBlock.earnedDowntimeBlocks - 1, maxBlock.earnedDowntimeBlocks],
        ]}
      />
      {highlightedBlock && (
        <Box
        ...
        >
          <Text size="large" weight="bold">
            {highlightedBlock.earnedDowntimeBlocks} blocks
          </Text>
          <Text>
            {moment(highlightedBlock.createdAt).format("MMM Do YYYY, h:mm a")}
          </Text>
        </Box>
      )}
    </Stack>
